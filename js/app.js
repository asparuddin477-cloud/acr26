...
window.printLogistik = function(pOverride) {
    ...
    var defaultMode = 'standard';
    try {
        defaultMode = localStorage.getItem('acr_logistik_print_mode') || 'standard';
    } catch(e) {}

    printWin.document.open();
    printWin.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Struk - ' + logCode + '</title>'
+ '<style id="pageStyle">'
+ (defaultMode === 'thermal' ? '@page{size:58mm auto;margin:0;}' : '@page{size:auto;margin:15mm;}')
+ '</style>'
+ '<style>'
+ '*{box-sizing:border-box;margin:0;padding:0;}'
+ 'body{font-family:Arial,Helvetica,sans-serif;color:#000;background:#dde3ec;padding:12px;transition:background .2s;}'
+ '.panel{max-width:440px;margin:0 auto 16px;background:#fff;border-radius:14px;box-shadow:0 4px 14px rgba(0,0,0,.15);padding:16px 20px;font-size:12px;color:#1e293b;}'
+ '.panel h3{font-size:16px;font-weight:800;margin-bottom:4px;color:#0f172a;}'
+ '.sub{color:#64748b;font-size:11px;margin-bottom:12px;line-height:1.4;}'
+ '.mode-label{font-size:11px;font-weight:bold;color:#475569;margin-bottom:6px;display:flex;align-items:center;gap:6px;}'
+ '.mode-group{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;}'
+ '.btn-mode{padding:9px 8px;border:1.5px solid #cbd5e1;background:#f8fafc;color:#334155;border-radius:8px;font-size:11px;font-weight:bold;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:5px;transition:all .15s;}'
+ '.btn-mode:hover{border-color:#94a3b8;background:#f1f5f9;}'
+ '.btn-mode.active{border-color:#2563eb;background:#eff6ff;color:#1d4ed8;box-shadow:0 0 0 1px #2563eb;}'
+ '.mode-hint{background:#f8fafc;border-left:3px solid #2563eb;padding:6px 10px;font-size:11px;color:#475569;border-radius:0 6px 6px 0;margin-bottom:14px;}'
+ '.btn-print{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:#16a34a;color:#fff;border:none;padding:11px;border-radius:10px;font-weight:bold;cursor:pointer;font-size:14px;box-shadow:0 2px 6px rgba(22,163,74,.35);transition:background .15s;}'
+ '.btn-print:hover{background:#15803d;}'
+ '/* TAMPILAN SCREEN TICKET */'
+ '.ticket-wrapper{display:flex;justify-content:center;align-items:flex-start;width:100%;padding:4px 0;}'
+ '.ticket{background:#fff;text-align:center;box-shadow:0 3px 12px rgba(0,0,0,.15);}'
+ '.cut-guide{display:none;font-size:8px;font-weight:bold;color:#64748b;letter-spacing:1px;margin-bottom:5px;text-transform:uppercase;}'
+ '.header{border-bottom:1px dashed #000;padding-bottom:4px;margin-bottom:6px;}'
+ '.event-title{font-weight:900;text-transform:uppercase;letter-spacing:.4px;}'
+ '.doc-title{font-weight:bold;text-transform:uppercase;color:#333;margin-top:2px;}'
+ '.qr-section{margin:5px 0 4px;text-align:center;display:flex;justify-content:center;}'
+ '#qrImg{display:block;margin:0 auto;image-rendering:pixelated;}'
+ '.log-code-label{font-weight:bold;color:#444;text-transform:uppercase;letter-spacing:1px;}'
+ '.log-code{font-weight:900;letter-spacing:2px;font-family:monospace;}'
+ '.info-section{border-top:1px dashed #000;border-bottom:1px dashed #000;padding:5px 0;margin:6px 0;text-align:left;}'
+ '.info-item{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:3px;line-height:1.35;}'
+ '.info-item:last-child{margin-bottom:0;}'
+ '.info-label{font-weight:bold;min-width:18mm;flex-shrink:0;color:#333;}'
+ '.info-value{font-weight:bold;text-align:right;flex:1;word-break:break-word;overflow-wrap:break-word;}'
+ '.bib-val{font-family:monospace;font-weight:900;color:#000;}'
+ '.footer{padding-top:4px;text-align:center;}'
+ '.footer p{color:#444;}'
+ '/* MODE STANDAR (PRINTER BIASA - DITENGAH KERTAS) */'
+ 'body.mode-standard .ticket{width:86mm;border:1.5px dashed #475569;border-radius:10px;padding:6mm 6mm;margin:0 auto;}'
+ 'body.mode-standard .cut-guide{display:block;}'
+ 'body.mode-standard .event-title{font-size:11px;}'
+ 'body.mode-standard .doc-title{font-size:8px;}'
+ 'body.mode-standard #qrImg{width:42mm;height:42mm;}'
+ 'body.mode-standard .log-code-label{font-size:8px;margin-top:4px;}'
+ 'body.mode-standard .log-code{font-size:18px;margin-top:2px;}'
+ 'body.mode-standard .info-section{font-size:9.5px;}'
+ 'body.mode-standard .bib-val{font-size:12px;}'
+ 'body.mode-standard .footer p{font-size:7.5px;}'
+ '/* MODE THERMAL (58MM / EPPOS) */'
+ 'body.mode-thermal .ticket{width:56mm;border:none;border-radius:0;padding:3mm 2mm;margin:0 auto;}'
+ 'body.mode-thermal .cut-guide{display:none;}'
+ 'body.mode-thermal .event-title{font-size:9px;}'
+ 'body.mode-thermal .doc-title{font-size:7px;}'
+ 'body.mode-thermal #qrImg{width:30mm;height:30mm;}'
+ 'body.mode-thermal .log-code-label{font-size:7px;margin-top:3px;}'
+ 'body.mode-thermal .log-code{font-size:15px;margin-top:1px;}'
+ 'body.mode-thermal .info-section{font-size:8px;}'
+ 'body.mode-thermal .bib-val{font-size:10px;}'
+ 'body.mode-thermal .footer p{font-size:6.5px;}'
+ '/* PRINT MEDIA QUERY */'
+ '@media print{'
+ '.no-print{display:none!important;}'
+ 'html,body{background:#fff!important;padding:0!important;margin:0!important;-webkit-print-color-adjust:exact;print-color-adjust:exact;}'
+ '/* Cetak Printer Biasa: Otomatis di tengah kertas */'
+ 'body.mode-standard{width:100%!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:flex-start!important;min-height:auto!important;}'
+ 'body.mode-standard .ticket-wrapper{display:flex!important;justify-content:center!important;align-items:center!important;width:100%!important;padding:0!important;}'
+ 'body.mode-standard .ticket{width:86mm!important;max-width:86mm!important;margin:20mm auto 0 auto!important;padding:5mm 5mm!important;border:1.5px dashed #000!important;box-shadow:none!important;page-break-inside:avoid!important;}'
+ 'body.mode-standard #qrImg{width:42mm!important;height:42mm!important;margin:3mm auto!important;}'
+ '/* Cetak Thermal Eppos */'
+ 'body.mode-thermal{width:56mm!important;margin:0 1mm!important;display:block!important;}'
+ 'body.mode-thermal .ticket-wrapper{display:block!important;width:56mm!important;}'
+ 'body.mode-thermal .ticket{width:56mm!important;padding:2mm 1.5mm!important;margin:0!important;border:none!important;box-shadow:none!important;}'
+ 'body.mode-thermal #qrImg{width:30mm!important;height:30mm!important;margin:0 auto!important;}'
+ '}'
+ '</style></head>'
+ '<body class="mode-' + defaultMode + '">'
+ '<div class="no-print panel">'
+ '<h3>Struk Logistik &mdash; ' + logCode + '</h3>'
+ '<div class="sub">Atur posisi cetak sesuai jenis printer yang Anda gunakan:</div>'
+ '<div class="mode-label">&#9881;&#65039; Pilihan Printer:</div>'
+ '<div class="mode-group">'
+ '<button id="btnStd" class="btn-mode ' + (defaultMode === 'standard' ? 'active' : '') + '" onclick="setMode(\'standard\')">&#128424;&#65039; Printer Biasa (Tengah)</button>'
+ '<button id="btnThm" class="btn-mode ' + (defaultMode === 'thermal' ? 'active' : '') + '" onclick="setMode(\'thermal\')">&#129534; Thermal 58mm (Eppos)</button>'
+ '</div>'
+ '<div id="hintText" class="mode-hint">'
+ (defaultMode === 'standard' 
    ? '<strong>Mode Printer Biasa Aktif:</strong> Struk dan QR Code otomatis diposisikan tepat di tengah kertas (A4 / F4 / Letter) lengkap dengan garis potong rapi.' 
    : '<strong>Mode Thermal 58mm Aktif:</strong> Khusus printer kasir roll seperti Eppos, Panda, VSC, dll.')
+ '</div>'
+ '<button class="btn-print" onclick="window.print()">&#128424;&#65039; Cetak Sekarang</button>'
+ '</div>'
+ '<div class="ticket-wrapper">'
+ '<div class="ticket">'
+ '<div class="cut-guide">&#9986;&#65039; GARIS POTONG KUPON LOGISTIK</div>'
...
+ '</div>'
+ '<div class="footer"><p>Serahkan kupon ini ke meja pengambilan logistik.</p></div>'
+ '</div>'
+ '</div>'
+ '<script>'
+ 'function setMode(m){'
+ '  document.body.className = "mode-" + m;'
+ '  document.getElementById("pageStyle").innerHTML = (m === "thermal") ? "@page{size:58mm auto;margin:0;}" : "@page{size:auto;margin:15mm;}";'
+ '  document.getElementById("btnStd").className = "btn-mode" + (m === "standard" ? " active" : "");'
+ '  document.getElementById("btnThm").className = "btn-mode" + (m === "thermal" ? " active" : "");'
+ '  document.getElementById("hintText").innerHTML = (m === "standard") '
+ '    ? "<strong>Mode Printer Biasa Aktif:</strong> Struk dan QR Code otomatis diposisikan tepat di tengah kertas (A4 / F4 / Letter) lengkap dengan garis potong rapi."'
+ '    : "<strong>Mode Thermal 58mm Aktif:</strong> Khusus printer kasir roll seperti Eppos, Panda, VSC, dll.";'
+ '  try { localStorage.setItem("acr_logistik_print_mode", m); } catch(e){}'
+ '}'
+ '</script>'
+ '</body></html>');
